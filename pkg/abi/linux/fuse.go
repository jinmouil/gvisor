...
type FUSEInitOut struct {
	...
	// CongestionThreshold is the threshold for congestion.
	// When the number of the pending background requests goes beyond
	// this threshold, the congestion begins.
	CongestionThreshold uint16

	// MaxWrite is the maximum size of a write buffer.
	// Minimum value is fuse/init.go:fuseMinMaxWrite.
	// Automatically adjusted to that minimum if the value from daemon is too small.
	MaxWrite uint32

	// TimeGran is the time granularity for mtime and ctime supported by the daemon.
	// Values should be power of 10 and unit is nanosecond.
	// 1 indicates full nanosecond granularity support.
	TimeGran uint32

	// MaxPages is the maximum number of pages for one write operation.
	// Maximum value is fuse/init.go:FUSE_MAX_MAX_PAGES.
	// Automatically adjusted to that maximum if the value from daemon is too large.
	MaxPages uint16
